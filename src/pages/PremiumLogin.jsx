...
export default function PremiumLogin() {
  ...
  async function onSubmit(e) {
    ...
    if (user) {
      if (user.isAdmin) {
        console.log(
          '[Login] Admin login successful, navigating to admin dashboard'
        );
        nav('/premium/admin');
        return;
      }
      ...
      <div style={{ textAlign: 'center', marginBottom: 10 }}>
        <div
          style={{
            ...
            fontWeight: 800,
            color: 'var(--accent)',
            textShadow: '0 0 16px rgba(90,224,255,.35)',
          }}
        >
          {count}
        </div>
      ...
      {showExpired ? (
        ...
        >
          ...
          <div style={{ fontSize: 16, marginBottom: 20, opacity: 0.9 }}>
            Your 7-day free access has expired. Please renew to continue your
            training.
          </div>
          <a
            href={WHATSAPP_LINK}
        ...
        >
          ...
          {!!err && (
            <div style={{ color: '#ff8b92', textAlign: 'center' }}>{err}</div>
          )}
