...
import {
  ...
  modules,
  getModulesAsync,
  getQuestionsForModuleAsync,
  getSavedAnswers,
  setSavedAnswers,
  clearSavedAnswers,
  getVideoAccess,
  activatePaidAccessBackend,
  getUsersBackendAsync,
  setProgressBackend,
  setSavedAnswersBackend,
  clearSavedAnswersBackend,
  verifyModuleAnswersBackend,
  getProgressBackend,
  signOut,
} from '../lib/premium';
...
export default function PremiumCourse() {
  ...
  const [prog, setProg] = useState(() =>
    user
      ? getProgress(user.id)
      : { unlocked: 1, completed: [false, false, false, false] }
  );
  ...
  function daysLeft(u) {
    ...
    const now = Date.now();
    const paidUntil = u?.paidAccessUntil
      ? new Date(u.paidAccessUntil).getTime()
      : null;
    if (paidUntil && paidUntil > now)
      return Math.ceil((paidUntil - now) / (24 * 60 * 60 * 1000));
  ...
  const WHATSAPP_LINK =
    'https://wa.me/918012202083?text=i%20am%20ready%20to%20pay%20Rs%20499%20/%205%20usdt%20please%20send%20me%20the%20QR%20or%20payment%20details.';
  ...
  useEffect(() => {
    ...
    const isDirectVideo =
      currentModule.src &&
      /\.(mp4|webm|ogg|mov)$/i.test(String(currentModule.src));
  ...
  async function submitQuiz(e) {
    e.preventDefault();
    const saved = user
      ? getSavedAnswers(user.id, courseModules[modIdx].id)
      : {};
    ...
    const res = await verifyModuleAnswersBackend(
      courseModules[modIdx].id,
      finalAnswers
    );
    ...
    if (!wasCompleted && wrong.length > 0) {
      alert(
        `❌ You have ${wrong.length} incorrect answer(s). Please review the highlighted questions and try again.`
      );
      if (!retakeMode) {
        setEnded(false);
    ...
    <div className="app-main" style={{ textAlign: 'center' }}>
      ...
      {!active ? (
        <div className="card expired-card">
          <div className="expired-title">🔒 Access Expired</div>
          <div className="expired-text">
            Your course access has expired. Renew your access to continue
            learning.
          </div>
          <button
            className="btn"
            onClick={activatePaid}
            style={{ marginTop: 10 }}
          >
        ...
        <>
          ...
          {(getVideoAccess(user.id) ||
            !prog.completed[courseModules.length - 1]) && (
              ...
              <div className="video-controls-row">
                {(() => {
                  ...
                  const timeRemaining = Math.max(
                    0,
                    REQUIRED_WATCH_TIME - watchTimer
                  );
                  const canStartQuiz = videoFullyWatched || isCompleted;
                  const progressPercent = isDirectVideo
                    ? pct
                    : Math.round((watchTimer / REQUIRED_WATCH_TIME) * 100);
                  ...
                  <>
                    <button
                      ...
                      title={
                        canStartQuiz
                          ? 'Start the quiz'
                          : `Please watch for ${timeRemaining}s more`
                      }
                    >
          ...
          {(() => {
            try {
              ...
              return (
                <div
                  className="card quiz-section-card"
                  style={{ marginTop: 20 }}
                >
                  <form onSubmit={submitQuiz} className="quiz-form">
                    <div className="quiz-header">
                      📘 Trendex Training – Assessment (Set {currentModule.id})
                    </div>
                    <div>
                      <strong>Answer all 25 questions</strong>
                    </div>
                    {(() => {
                      try {
                        const saved = user
                          ? getSavedAnswers(user.id, currentModule.id)
                          : {};
                        const reviewMode =
                          prog.completed[modIdx] && !retakeMode;

                        if (!qs || qs.length === 0) {
                          return (
                            <div className="quiz-feedback-error">
                              Error: No questions loaded. Please refresh the
                              page.
                            </div>
                          );
                        }
                        ...
                        return (
                          <>
                            ...
                            {validated && wrongIds.length > 0 && (
                              <div className="quiz-feedback-error">
                                ❌ Some answers are incorrect. Review the
                                highlighted choices (red = wrong answer).
                              </div>
                            )}
                            {!reviewMode ? (
                              <button
                        ...
                        return (
                          <div className="quiz-feedback-error">
                            Error rendering quiz: {err.message}
                          </div>
                        );
                      }
                    })()}
          ...
          {prog.completed[modules.length - 1] && (
            <div className="card course-completed-card">
              ...
              <Link className="btn" to="/premium/certificate">
                Get Certificate →
              </Link>
            </div>
          )}
          <p className="video-path-note">
            Please make an notes of this videos and Q & A,
            <br />
            Must needed Knowledge's to Learn more and Earn more,
            <br /> This section Free to access only for First 7 Days{' '}
          </p>
          <button
            className="btn secondary"
            onClick={() => {
